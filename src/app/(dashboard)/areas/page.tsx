  ...
  return (
    <div style={{ display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--r)", overflow: "hidden" }}>
        {/* Header */}
        <div style={{ display: "grid", gridTemplateColumns: "220px 1fr 90px 80px 80px", padding: "12px 20px", background: "var(--surface-alt)", borderBottom: "1px solid var(--border-light)", fontSize: 12, fontWeight: 600, color: "var(--text-3)", letterSpacing: "0.3px", textTransform: "uppercase" }}>
          <div>Área</div>
        ...
        {filteredAreas.length === 0 ? (
          <div style={{ padding: 64, textAlign: "center", color: "var(--text-3)", fontSize: 14 }}>
          ...
          return (
            <Link key={kpi.id} href={`/areas/${kpi.areaId}`} style={{ display: "grid", gridTemplateColumns: "220px 1fr 90px 80px 80px", alignItems: "center", padding: "14px 20px", borderBottom: i < filteredAreas.length - 1 ? "1px solid var(--border-light)" : "none", background: isBad ? "#fef2f2" : "transparent", textDecoration: "none", transition: "background 0.1s" }} onMouseEnter={e => { if(!isBad) e.currentTarget.style.background = "var(--surface-alt)" }} onMouseLeave={e => { if(!isBad) e.currentTarget.style.background = "transparent" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <div style={{ width: 10, height: 10, borderRadius: "50%", background: areaColor }} />
                <span style={{ fontSize: 15, fontWeight: 500, color: "var(--text-1)" }}>{kpi.area?.name}</span>
              </div>
              ...
              <div style={{ display: "flex", alignItems: "center", paddingRight: 20 }}>
                ...
                <span style={{ fontSize: 12, fontWeight: 600, color: areaColor, whiteSpace: "nowrap", width: 32 }}>{rate}%</span>
              </div>
